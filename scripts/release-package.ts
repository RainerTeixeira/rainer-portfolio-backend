 ...
 */
function deployToProduction(): void {
  try {
    logStep('🚀', 'Fazendo deploy para produção...');
    execSync('pnpm sam:deploy:prod', { stdio: 'inherit' });
    logSuccess('Deploy para produção concluído');
  } catch (error) {
 ...
 */
function main(): void {
  logHeader('Sistema de Release - rainer-portfolio-backend');
  ...
  console.log(`🚀 Deploy: AWS Lambda via SAM`);
  console.log(`📋 GitHub: https://github.com/RainerTeixeira/rainer-portfolio-backend/releases/tag/v${packageInfo.version}`);
