...
function loadMemories() {
  ...
  if (existsSync(technicalPath)) {
    try {
      ...
      console.warn('⚠️ Erro ao carregar technical-details.json:', error.message);
    }
  }
...
function displaySummary(memories) {
  ...
  if (memories.initial) {
    const project = memories.initial.entities?.find(
      (e) => e.fullName === 'rainer-portfolio-backend'
    );
    if (project) {
      console.log(`\n📋 Projeto: ${project.fullName}`);
      console.log(`   Última atualização: ${memories.initial.lastModified || 'N/A'}`);
    }
  }

  if (memories.technical) {
    console.log(`\n🔧 Endpoints: ${memories.technical.technicalDetails?.endpoints?.total || 'N/A'}`);
    console.log(`   Última atualização: ${memories.technical.lastModified || 'N/A'}`);
  }
  ...
  if (memories.code) {
    ...
    if (project) {
      ...
      console.log(`   Status: ${project.observations?.find((o) => o.includes('Status:')) || 'N/A'}`);
    }
  }
