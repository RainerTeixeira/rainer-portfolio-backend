...
function formatMemory(memory: any, type: string): string {
  ...
  if (type === 'initial' && memory.entities) {
    const project = memory.entities.find((e: any) => e.fullName === 'rainer-portfolio-backend');
    if (project) {
    ...
    output += `Endpoints: ${memory.technicalDetails.endpoints?.total || 'N/A'}\n`;
    output += `Última atualização: ${memory.lastModified || 'N/A'}\n\n`;
    if (memory.technicalDetails.organization) {
      output += 'Estrutura:\n';
      Object.entries(memory.technicalDetails.organization.structure || {}).forEach(([key, value]) => {
        output += `  • ${key}: ${value}\n`;
      });
    }
  } else if (type === 'code' && memory.entities) {
    const project = memory.entities.find((e: any) => e.fullName === 'Blog Backend Serverless');
    if (project) {
      output += `Projeto: ${project.fullName}\n`;
