# 🧠 Sistema de Memórias do Projeto - Backend

Este diretório contém a configuração e os scripts para o sistema de memórias do projeto `rainer-portfolio-backend`. O objetivo é fornecer um contexto rico e atualizado para ferramentas de IA, como o Cursor, e para qualquer desenvolvedor que precise entender rapidamente a arquitetura, as decisões técnicas e o estado atual do projeto.
...
- **Via Script**: Execute `npm run memory:update` para atualizar todas as memórias.
...
```json
{
  "portfolio-backend-memory": {
    "command": "node",
    "args": ["C:\\Desenvolvimento\\rainer-portfolio-backend\\.cursor\\mcp-memory-server.mjs"],
    "env": {
      "MEMORY_DIR": "C:\\Desenvolvimento\\rainer-portfolio-backend\\docs\\.memories",
      "PROJECT_ROOT": "C:\\Desenvolvimento\\rainer-portfolio-backend"
    }
  }
}
```
