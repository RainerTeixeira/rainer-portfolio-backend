 ...
 *
 */
import { Injectable, UnauthorizedException, BadRequestException, InternalServerErrorException, ConflictException, Logger } from '@nestjs/common';
...
export class AuthService {
   ...
   */
  private async checkNicknameAvailability(nickname: string, currentUserId?: string): Promise<boolean> {
    try {
      this.logger.debug(`[checkNicknameAvailability] Verificando disponibilidade de nickname: ${nickname}`, {
        nickname,
        currentUserId,
      });
        ...
        if (responsePreferred.Users && responsePreferred.Users.length > 0) {
          ...
          if (user.Attributes) {
            ...
            if (currentUserId && sub === currentUserId) {
              this.logger.debug(`[checkNicknameAvailability] Nickname pertence ao próprio usuário, disponível`);
              return true;
            }
            
            // Se não é o próprio usuário, não está disponível
            this.logger.debug(`[checkNicknameAvailability] Nickname já está em uso por outro usuário`);
            return false;
          }
        }
      } catch (error) {
        this.logger.warn(`[checkNicknameAvailability] Erro ao buscar por preferred_username: ${(error as Error).message}`);
      ...
      try {
        ...
        this.logger.warn(`[checkNicknameAvailability] Erro ao buscar por nickname: ${(error as Error).message}`);
      ...
      this.logger.error(`[checkNicknameAvailability] Erro ao verificar disponibilidade de nickname: ${err.message}`, err.stack);
  ...
  async verifyEmailChange(cognitoSub: string, code: string) {
    try {
      const { CognitoIdentityProviderClient, VerifyUserAttributeCommand } = await import('@aws-sdk/client-cognito-identity-provider');
  ...
  async login(data: LoginData): Promise<LoginResponse> {
    try {
      ...
      this.logger.debug(`[Login] ✅ Token decodificado: sub=${payload.sub}, email=${payload.email}`);
      ...
      if (!user) {
        ...
        try {
          ...
          if (response.UserCreateDate) {
            ...
            this.logger.log(`✅ createdAt sincronizado com Cognito no login: ${cognitoCreatedAt.toISOString()}`);
          }
        } catch (error) {
          this.logger.warn(`Não foi possível buscar data de criação do Cognito: ${(error as Error).message}`);
        }
        
        const email = payload.email || `${payload.sub.replace(/[^a-zA-Z0-9]/g, '')}@temporary.email`;
        this.logger.debug(`[Login] Criando usuário no MongoDB: email=${email}, fullName=${(payload.name as string) || 'Usuário'}`);
      ...
      return {
        ...
        user: {
          ...
          email: payload.email || (user as { email?: string }).email || '',
          avatar: user.avatar,
          bio: user.bio,
  ...
  async register(data: RegisterData): Promise<RegisterResponse> {
    try {
      ...
      const baseUsername = (data.fullName || 'user')
        .trim()
        .toLowerCase()
        .replace(/\s+/g, '_')
        .replace(/[^a-z0-9_.-]/g, '')
      ...
      for (;;) {
        ...
        try {
          ...
          throw new InternalServerErrorException('Erro ao verificar disponibilidade do nome de usuário');
        }
      }

      // 3) Nickname obrigatório e armazenado como atributo
      const cleanNickname = (data.nickname || '')
        .toLowerCase()
        .replace(/[^a-z0-9_.-]/g, '');
      ...
      if (cognitoResponse.CodeDeliveryDetails) {
        ...
        try {
          ...
          this.logger.log(`✅ Código de confirmação enviado com sucesso via fallback para: ${data.email}`);
        } catch (resendError) {
          const resendErr = resendError as Error & { fullName?: string };
          this.logger.error(
            `❌ Fallback também falhou: ${resendErr.name} - ${resendErr.message}`
          );
          this.logger.error(
      ...
      this.logger.log(`Usuário registrado no Cognito: ${candidate} (${cognitoSub}, Confirmado: ${cognitoResponse.UserConfirmed})`);
      ...
      try {
        ...
        if (response.UserCreateDate) {
          cognitoCreatedAt = response.UserCreateDate;
          this.logger.log(`✅ createdAt sincronizado com Cognito no registro: ${cognitoCreatedAt.toISOString()}`);
        }
      } catch (error) {
        this.logger.warn(`Não foi possível buscar data de criação do Cognito: ${(error as Error).message}`);
  ...
  async forgotPassword(data: ForgotPasswordData) {
    try {
      ...
      if (err.name === 'UserNotFoundException' || err.name === 'InvalidParameterException') {
        ...
        this.logger.warn(`Tentativa de recuperação de senha para email não encontrado: ${data.email}`);
        return { 
          success: true, 
          message: 'Se o email estiver cadastrado e verificado, você receberá um código de recuperação em instantes.' 
        };
  ...
  async resendConfirmationCode(email: string) {
    try {
      ...
      try {
        ...
        if (firstErr.name === 'NotAuthorizedException' && 
        ...
        if (username !== email && (firstErr.name === 'UserNotFoundException' || firstErr.name === 'NotAuthorizedException')) {
          this.logger.debug(`Tentativa com username falhou, tentando novamente com email como username`);
          try {
            ...
            if (emailErr.name === 'NotAuthorizedException' && 
      ...
      if (err.name === 'UserNotFoundException' || err instanceof BadRequestException) {
        throw err instanceof BadRequestException 
          ? err 
          : new BadRequestException('Usuário não encontrado com este email. Verifique se o cadastro foi concluído.');
      }
  ...
  async verifyEmailAdmin(identifier: string) {
    try {
      ...
      this.logger.log(`E-mail verificado administrativamente para usuário: ${cognitoUsername} (${emailAttribute.Value})`);
      ...
      try {
        ...
        if (confirmErr.name === 'NotAuthorizedException' && confirmErr.message?.includes('already confirmed')) {
          ...
          this.logger.warn(`Não foi possível confirmar o usuário (pode já estar confirmado): ${confirmErr.message}`);
        }
      }
  ...
  async changeNickname(cognitoSub: string, newNickname: string) {
    try {
      ...
      if (!/^[a-zA-Z0-9]+$/.test(newNickname)) {
        throw new BadRequestException('Nickname deve conter apenas letras e números (sem @, underscore ou outros caracteres especiais)');
      }
      ...
      if (!username) {
        this.logger.warn(`Username não encontrado via getUsernameBySub, tentando usar cognitoSub diretamente: ${cognitoSub}`);
        username = cognitoSub;
      }
      ...
      try {
        ...
        if (updateErr.name === 'UserNotFoundException' && username === cognitoSub) {
          this.logger.warn(`Falhou ao atualizar com cognitoSub, tentando buscar email do usuário...`);
          ...
          if (user && user.email) {
            ...
            try {
              ...
              this.logger.error(`Erro ao atualizar nickname usando email: ${(emailError as Error).message}`);
              throw new InternalServerErrorException(
      ...
      this.logger.error(`Erro ao alterar nickname para cognitoSub ${cognitoSub}: ${err.message}`, err.stack);
  ...
  private decodeToken(token: string): JwtPayload {
    try {
      ...
      this.logger.error('Erro ao decodificar token', error instanceof Error ? error.stack : String(error));
  ...
  async handleOAuthCallback(
    ...
    this.logger.debug(`Provider: ${provider}, Code presente: ${!!code}, RedirectUri: ${redirectUriOverride}`);
    ...
    if (codeData?.processing) {
      this.logger.warn('[AuthService] ⚠️  Código já está sendo processado, ignorando requisição duplicada');
      throw new BadRequestException('Código já está sendo processado. Aguarde a primeira requisição completar.');
    }
    
    if (codeData && (Date.now() - codeData.timestamp) < 60000) {
      this.logger.warn('[AuthService] ⚠️  Código já foi usado recentemente');
      throw new BadRequestException('Código de autorização já foi usado. Por favor, faça login novamente.');
    }
    ...
    try {
      ...
      if (state) {
        try {
          const decoded = JSON.parse(Buffer.from(state, 'base64url').toString('utf8')) as { p?: string };
      ...
      const payload = this.decodeToken(idToken) as JwtPayload;
      this.logger.debug(`[AuthService] Payload: sub=${payload.sub}, email=${payload.email}, name=${payload.name || payload.fullName}`);
      ...
      if (!user) {
        ...
        try {
          ...
          if (err.message?.includes('Já existe usuário com este nome')) {
            this.logger.warn('[AuthService] Erro de nome duplicado, tentando buscar usuário existente...');
      ...
      try {
        ...
        try {
          ...
          if (userResponse.UserAttributes) {
            const nicknameAttr = userResponse.UserAttributes.find(attr => attr.Name === 'nickname');
            const hasNickname = !!(nicknameAttr?.Value && nicknameAttr.Value.trim().length > 0);
            needsNickname = !hasNickname;
            
            this.logger.debug(`[AuthService] Verificação de nickname: hasNickname=${hasNickname}, needsNickname=${needsNickname}`);
            ...
            if (needsNickname && user) {
              ...
              this.logger.debug(`[AuthService] Nickname gerado automaticamente: ${autoGeneratedNickname}`);
              
              // Verificar disponibilidade antes de criar
              const isAvailable = await this.checkNicknameAvailability(autoGeneratedNickname, payload.sub);
              if (!isAvailable) {
                ...
                this.logger.debug(`[AuthService] Nickname ajustado (não disponível): ${autoGeneratedNickname}`);
              }
              ...
              try {
                ...
                if (username) {
                  ...
                  const updateCommand = new AdminUpdateUserAttributesCommand({
                    ...
                    Username: username,
                    UserAttributes: [
                      { Name: 'nickname', Value: autoGeneratedNickname },
                    ],
                  });
                  
                  await client.send(updateCommand);
                  this.logger.log(`[AuthService] ✅ Nickname criado automaticamente: ${autoGeneratedNickname}`);
                  needsNickname = false; // Já foi criado
                }
              } catch (updateError) {
                this.logger.warn(`[AuthService] Erro ao criar nickname automaticamente: ${(updateError as Error).message}`);
          ...
          this.logger.warn(`[AuthService] Erro ao buscar usuário diretamente, tentando ListUsers: ${(getUserError as Error).message}`);
        ...
        this.logger.warn(`[AuthService] Erro ao verificar nickname no OAuth: ${(error as Error).message}`);
      ...
      const result = {
        ...
        user: {
          ...
          cognitoSub: user.cognitoSub || payload.sub,
          fullName: user.fullName,
          email: payload.email || (user as unknown as { email?: string }).email || '',
          avatar: user.avatar,
          bio: user.bio,
          website: user.website,
          socialLinks: user.socialLinks,
      ...
      this.processedCodes.delete(code);
      this.logger.log('[AuthService] ❌ Erro no callback, código removido do cache para permitir retry');
      ...
      throw new InternalServerErrorException(`Erro ao processar autenticação OAuth: ${err.message}`);
    }
  }
  ...
  private async exchangeCognitoCode(
    code: string,
    redirectUriOverride?: string,
  ): Promise<{ access_token: string; refresh_token?: string; id_token: string; token_type?: string; expires_in?: number }> {
    ...
    if (!domain || !clientId || !redirectUri) {
      throw new InternalServerErrorException('Configuração Cognito incompleta (COGNITO_DOMAIN/CLIENT_ID/OAUTH_REDIRECT_SIGN_IN)');
    }
    ...
    const data = await response.json() as { id_token?: string; access_token?: string; refresh_token?: string; token_type?: string; expires_in?: number };
